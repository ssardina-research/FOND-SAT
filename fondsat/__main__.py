...
def main():
    ...
    args_parser.add_argument("path_domain", help="Path to domain file (pddl)")
    args_parser.add_argument(
        "path_instance", help="Path to problem instance file (pddl)"
    )
    ...
    for i in range(params["start"], params["end"] + 1):
        ...
        print(
            "Will now call SAT solver with {:d}MB and {:d} seconds limits".format(
        ...
        print(
            "Done solver. Round time: {:f}".format(end_solver_time - start_solver_time)
        )
        ...
        if result:
            ...
            if not draw_policy and print_policy:
                ...
                with open(file_name, "w+") as f:
                    f.write(out_txt)
                draw(file_name, "controller.dot")
        elif (
            result is None
        ):  # clean-up whatever aux files were generated for this iteration
            clean(
                {
                    name_formula_file,
                    name_output_satsolver,
