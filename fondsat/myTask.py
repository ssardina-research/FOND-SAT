...
class MyTask:
    def __init__(self):
        ...
        self.relevant_actions_atom_aname = (
            {}
        )  # dictionary: (atom, a_name) --> relevant actions
        ...
        for a in self.action_names:
            ...
            for a2 in self.action_names:
                if a2 == a:
                    continue
                if (
                    self.get_action_name(a).split("_DETDUP_")[0]
                    == self.get_action_name(a2).split("_DETDUP_")[0]
                ):
